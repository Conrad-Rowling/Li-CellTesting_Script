...
uint8 PGA_4_initVar = 0u; /**< Describes the init state of the component */

uint32 PGA_4_internalGainPower = 0u; /**< Stores component Gain, Power and Enable values */
...
*******************************************************************************/
const uint32 PGA_4_compTab[PGA_4_COMP_TAB_HEIGHT][PGA_4_COMP_TAB_WIDTH] =
{
#if (PGA_4_CHECK_OUTPUT_MODE) /* Class AB */
...
void PGA_4_Init(void)
{
    ...
    PGA_4_internalGainPower = PGA_4_DEFAULT_GAIN_POWER;
    
    /* Set default register values */
    PGA_4_CTB_CTRL_REG |= PGA_4_DEFAULT_CTB_CTRL;
    PGA_4_OA_RES_CTRL_REG &= (uint32) ~PGA_4_DEFAULT_OA_RES_CTRL_MASK;
    PGA_4_OA_RES_CTRL_REG |= PGA_4_DEFAULT_OA_RES_CTRL;
    
    /* Check if component has been enabled */
    if((PGA_4_OA_RES_CTRL_REG & PGA_4_OA_PWR_MODE_MASK) != 0u)
    {
...
*  previous PGA_4_Stop() call).
*  Usually it is called in PGA_4_Start().
*  
*******************************************************************************/
void PGA_4_Enable(void)
{
    PGA_4_OA_RES_CTRL_REG |= PGA_4_GET_POWER;
}
...
****************************************************************************//**
*
*  Enables the amplifier operation (calls PGA_4_Enable).
*  Also on the first call (after the system reset) initializes all the component
*  related registers with default values (calls PGA_4_Init).
*
*  \globalvars
*   The PGA_4_initVar variable is used to indicate initial
*   configuration of this component. The variable is initialized to zero (0u)
*   and set to one (1u) at the first time PGA_4_Start() is called.
*   This allows to enable the component without re-initialization in all
*   subsequent calls of the PGA_4_Start() routine.
*
*******************************************************************************/
...
{ 
    PGA_4_OA_RES_CTRL_REG &= (uint32)~PGA_4_OA_PWR_MODE_MASK;
}
...
*   * PGA_4_LOW - The lowest power consumption.
*   * PGA_4_MED - The middle setting.
*   * PGA_4_HIGH - The highest amplifier bandwidth.
*
*  \internal
*   The PGA_4_internalGainPower variable is used to store the
...
void PGA_4_SetPower(uint32 powerLevel)
{
    ...
    /* Save the powerLevel */
    PGA_4_internalGainPower &= (uint32) ~PGA_4_OA_PWR_MODE_MASK;
    PGA_4_internalGainPower |= powerLevel & PGA_4_OA_PWR_MODE_MASK;
    
    /* Save the rest of register bitfields */
    locTmp = PGA_4_OA_RES_CTRL_REG &
        (uint32)~(PGA_4_OA_PWR_MODE_MASK | PGA_4_C_FB_MASK);
           
    /* Set the power and the feedback capacitor values into the control register */
    PGA_4_OA_RES_CTRL_REG  = locTmp | PGA_4_GET_POWER | PGA_4_GET_C_FB;
    
    /* Set the OA compensation capacitor value */
    PGA_4_OA_COMP_TRIM_REG = PGA_4_GET_OA_COMP_TRIM;
}
...
*   * PGA_4_GAIN_1_00 - gain 1.00.
*   * PGA_4_GAIN_1_42 - gain 1.42.
*   * PGA_4_GAIN_2_00 - gain 2.00.
*   * PGA_4_GAIN_2_78 - gain 2.78.
*   * PGA_4_GAIN_4_00 - gain 4.00.
*   * PGA_4_GAIN_5_82 - gain 5.82.
*   * PGA_4_GAIN_8_00 - gain 8.00.
*   * PGA_4_GAIN_10_7 - gain 10.7.
*   * PGA_4_GAIN_16_0 - gain 16.0.
*   * PGA_4_GAIN_21_3 - gain 21.3.
*   * PGA_4_GAIN_32_0 - gain 32.0.
*
*  \internal
*   The PGA_4_internalGainPower variable is used to store the
*   current gain and power level to set appropriate compensation settings.
...
void PGA_4_SetGain(uint32 gainLevel)
{
    ...
    /* Save the gainLevel */
    PGA_4_internalGainPower &= (uint32) ~PGA_4_RES_TAP_MASK;
    PGA_4_internalGainPower |= ((uint32)(gainLevel << PGA_4_RES_TAP_SHIFT)) &
                                                                 PGA_4_RES_TAP_MASK;
    /* Save the rest of register bitfields */
    locTmp = PGA_4_OA_RES_CTRL_REG &
        (uint32)~(PGA_4_RES_TAP_MASK | PGA_4_C_FB_MASK);
    
    /* Set the gain and the feedback capacitor values into the control register */
    PGA_4_OA_RES_CTRL_REG = locTmp | (PGA_4_internalGainPower &
        PGA_4_RES_TAP_MASK) | PGA_4_GET_C_FB;
    
#if (PGA_4_VREF_MODE == PGA_4_VREF_INTERNAL)
    if(PGA_4_GAIN_1_00 == gainLevel)
    {
        /* Disconnect the resistor matrix bottom from the internal Vss */
        PGA_4_OA_SW_CLEAR_REG = PGA_4_RBOT_TO_VSSA;
    }
    else
    {
        /* Connect the resistor matrix bottom to the internal Vss */
        PGA_4_OA_SW_REG = PGA_4_RBOT_TO_VSSA;
    }
#endif /* Vref Internal */
...
/*******************************************************************************
* Function Name: PGA_4_PumpControl
****************************************************************************//**
*
*  Allows the user to turn the amplifier's boost pump on or off.
*  By Default the PGA_4_Init() function turns the pump on.
...
*   * PGA_4_BOOST_OFF - Turn off the pump.
*   * PGA_4_BOOST_ON  - Turn on the pump.
*
**********************************************************************************/
void PGA_4_PumpControl(uint32 onOff)
{
    if(onOff == PGA_4_BOOST_ON)
    {
        PGA_4_OA_RES_CTRL_REG |= PGA_4_OA_PUMP_EN;
    }
    else
    {
        PGA_4_OA_RES_CTRL_REG &= (uint32)~PGA_4_OA_PUMP_EN;
    }
}
