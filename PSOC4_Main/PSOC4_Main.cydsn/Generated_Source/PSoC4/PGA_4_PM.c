...
*
*  \internal
*   The PGA_4_backup.enableState is modified 
*   depending on the enable state of the block before entering to sleep mode.
*
*******************************************************************************/
void PGA_4_Sleep(void)
{
    #if(!PGA_4_CHECK_DEEPSLEEP_SUPPORT)
        if((PGA_4_OA_RES_CTRL_REG & PGA_4_OA_PWR_MODE_MASK) != 0u)
        {
            PGA_4_internalGainPower |= PGA_4_ENABLED;
            PGA_4_Stop();
        }
        else /* The component is disabled */
        {
            PGA_4_internalGainPower &= (uint32) ~PGA_4_ENABLED;
        }
        /* Save the configuration */
        PGA_4_SaveConfig();
    #endif /* (PGA_4_CHECK_DEEPSLEEP_SUPPORT) */
}
...
*
*  \internal
*   The PGA_4_backup.enableState is used to 
...
void PGA_4_Wakeup(void)
{
        ...
        if((PGA_4_internalGainPower & PGA_4_ENABLED) != 0u)
        {
            PGA_4_Enable();
