...
uint8 Opamp_3_initVar = 0u; /* Defines if component was initialized */
static uint32 Opamp_3_internalPower = 0u; /* Defines component Power value */
...
{
    Opamp_3_internalPower = Opamp_3_POWER;
    Opamp_3_CTB_CTRL_REG = Opamp_3_DEFAULT_CTB_CTRL;
    Opamp_3_OA_RES_CTRL_REG = Opamp_3_DEFAULT_OA_RES_CTRL;
    Opamp_3_OA_COMP_TRIM_REG = Opamp_3_DEFAULT_OA_COMP_TRIM_REG;
}
...
{
    Opamp_3_OA_RES_CTRL_REG |= Opamp_3_internalPower | \
                                        Opamp_3_OA_PUMP_EN;
}
...
{
    Opamp_3_OA_RES_CTRL_REG &= ((uint32)~(Opamp_3_OA_PWR_MODE_MASK | \
                                                   Opamp_3_OA_PUMP_EN));
}
...
* Parameters:
*  power: power levels.
*   Opamp_3_LOW_POWER - Lowest active power
*   Opamp_3_MED_POWER - Medium power
*   Opamp_3_HIGH_POWER - Highest active power
*
* Return:
*  None
*
...
void Opamp_3_SetPower(uint32 power)
{
    ...
    Opamp_3_internalPower = Opamp_3_GET_OA_PWR_MODE(power);
    tmp = Opamp_3_OA_RES_CTRL_REG & \
           (uint32)~Opamp_3_OA_PWR_MODE_MASK;
    Opamp_3_OA_RES_CTRL_REG = tmp | Opamp_3_internalPower;
}


/*******************************************************************************
* Function Name: Opamp_3_PumpControl
********************************************************************************
*
* Summary:
...
*   Opamp_3_PUMP_OFF - Turn off the pump
*   Opamp_3_PUMP_ON - Turn on the pump
*
* Return:
*  None
*
...
void Opamp_3_PumpControl(uint32 onOff)
{
    ...
    if(0u != onOff)
    {
        Opamp_3_OA_RES_CTRL |= Opamp_3_OA_PUMP_EN;    
    }
    else
    {
        Opamp_3_OA_RES_CTRL &= (uint32)~Opamp_3_OA_PUMP_EN;
    }
}
